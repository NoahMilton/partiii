...
variablelabel = "modulus"
unit = ""
savelocation = "gravity_packing"    # gravity_packing or tapping_packing
errors = 1          # Error bars:   on = 1, off = 0 
plot_X_Fu = 1      # X. Fu data:   on = 1, off = 0
...
for i in range(len(filelist)):  # Get data from files, remove 0 values
    ...
    av_delta.append(np.average(delta)*100/p_size) # delta overlap in %
    delta_err.append((np.std(delta)*100/p_size)/np.sqrt(len(delta)))
    average_cforce.append(np.average(c_force[i]))
    average_cforce_err.append(np.std(c_force[i])/np.sqrt(len(c_force[i])))
    average_nforce.append(np.average(n_force[i]))
    average_nforce_err.append(np.std(n_force[i])/np.sqrt(len(n_force[i])))

## Average Height, into z_av and z_std
for i in range(len(filelist)):
    z = average_z(filelist[i])
    z_av.append(z[0]/p_size)
    z_std.append(z[1]/p_size)
...
remove_end = 0          # remove from high end
remove_beginning = 1    # remove from low end
...
if remove_end > 0:
    for i in range (remove_end):
        ...
        delta_err = np.delete(delta_err, [final_index])
        ...
        average_cforce_err = np.delete(average_cforce_err, [final_index])
        average_nforce = np.delete(average_nforce,[final_index])
        average_nforce_err = np.delete(average_nforce_err, [final_index])
...
if errors ==1:
    plt.errorbar(values, average_cforce, fmt = 'x', yerr = average_cforce_err, elinewidth = 1, capsize = 1, label = "Contact Force")
    plt.errorbar(values, average_nforce, fmt = '.', yerr = average_nforce_err, elinewidth = 1, capsize = 1, label = "Normal Force")
else:
...
fig1.savefig('/Users/Noah/python/'+savelocation+'/' + 'cF_' + variable + str(values[0]) +
             "_to_" + str(values[-1]) + "_" + str(N) + '.png', dpi=400)

fig2.savefig('/Users/Noah/python/'+savelocation+'/' + 'nF_' + variable +  str(values[0]) +
             "_to_" + str(values[-1]) + "_" + str(N) + '.png', dpi=400)

fig3.savefig('/Users/Noah/python/'+savelocation+'/' + 'delta_' + variable + str(values[0]) +
             "_to_" + str(values[-1]) + "_" + str(N) + errortext + '.png', dpi=400)

fig4.savefig('/Users/Noah/python/'+savelocation+'/' + 'cF_nF_' + variable + str(values[0]) +
             "_to_" + str(values[-1]) + "_" + str(N) + errortext + '.png', dpi=400)

fig5.savefig('/Users/Noah/python/'+savelocation+'/' + 'z_' + variable + str(values[0]) +
             "_to_" + str(values[-1]) + "_" + str(N) + errortext + '.png', dpi=400)

fig6.savefig('/Users/Noah/python/'+savelocation+'/' + 'RDF_' + variable + str(values[0])+ 
             "_to_" + str(values[-1]) + "_" + str(N) + Fu + '.png', dpi=400)

fig7.savefig('/Users/Noah/python/'+savelocation+'/' + 'CN_' + variable + str(values[0])+ 
             "_to_" + str(values[-1]) + "_" + str(N) + errortext + '.png', dpi=400)
